...
def get_service_version(repo_name):
    ...
    if repo_name == "ha-pepper":
        try:
            req = urllib.request.Request(
                "https://sysupgrade.pepper.org/api/v1/latest", headers=headers
            )
            with urllib.request.urlopen(req, timeout=5) as response:
                data = json.loads(response.read().decode("utf-8"))
                return data["latest"][0]
        except Exception as e:
            print(f"Error fetching Pepper version: {e}")
            return "25.12.4"
...
def clean_and_update_template(file_path, integration_version, ha_version, repo_name):
    ...
    for i in range(2, len(blocks), 2):
        ...
        # 3. Service versions
            ...
            (field_id == "pepper_version" and repo_name == "ha-pepper")
